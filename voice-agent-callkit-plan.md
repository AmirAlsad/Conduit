...
**Add / Edit agent (the sheet).** This is the bring-your-own configuration surface. Name; description (shown as the row's second line, like a contact label); optional avatar with an initials/monogram fallback; the connection (transport, URL, token, or pairing endpoint); the system-contact mirror toggle; and a **test-connection** action that does a quick connect-and-disconnect so a stale token fails at the desk, not on the highway. Each toggle gets a footer explanation — the system-contact toggle's footer notes that turning it on lets you say "Hey Siri, call Jarvis," shows the agent in your Contacts, and puts its photo on the call screen.
