...
> **Current state.** The app is built through **M6**: the M0 foundation (protocol seams + fakes, SwiftData models, `AppEnvironment`), the M1 call state machine (`CallSessionCoordinator`, reconnection, spoken state), the real Daily transport (M2), the CallKit + audio-session seam (M3), the Keychain real + permission-free Add-to-Contacts (WS-4 + follow-ups), all six `Features/` modules + `Shared/` components (WS-5), the device-verified M5 realistic target (pairing call, two-way audio, mute, in-app routing, contact enrichment), and the native **LiveKit** transport (M6) — wired through `AppEnvironment.live()` (persistent store + real services; CallKit/Daily/LiveKit fall back to fakes in the simulator). LiveKit uses manual audio (`AudioManager.setEngineAvailability` gated on CallKit), so on LiveKit calls the **native call-screen route button works** (Daily can't — see `bugs.md`). Since M6: Recents **swipe-to-delete**; an **audio-interruption seam** (`AudioInterruptionObserving`) that pauses/resumes the mic on Siri/phone-call/nav takeovers; and **agent-initiated inbound calls** (VoIP push → CallKit → answer → reuse the outgoing connect path; see [`docs/INBOUND_CALLS.md`](./docs/INBOUND_CALLS.md)) — with **ring-status receipts** (a `status_url` in the push makes the app POST back answered / declined / busy / suppressed_by_focus; `RingStatusReporting` seam, `/inbound/status` on the engine) and token registration on agent **save** as well as launch (`InboundRegistering`). Live interruption/ducking audio remains **device-only/unverified**. The **LiveKit path is device-verified** (pairing connect, two-way audio, native route button, reconnection — against the deployed reference engine, June 2026), and the **inbound path is device-verified end-to-end** (June 2026, LiveKit **and Daily** agents — including ring-status receipts, toggle-save registration, unknown-agent graceful end, and immediate end-call teardown on hangup): cold ring → answer → two-way audio, decline → Recents, inline creds, Focus/DND-filtered rings logged as missed with a quiet local notification (`MissedCallNotifier`, provisional auth), busy pushes report-then-end (a silently ignored VoIP push gets the app killed — see `bugs.md`), and rings surviving a backend redeploy (volume-backed registry). The reference engine's inbound stack (APNs sender, `/inbound/register`, `/admin/ring`, `scripts/ring.py`) lives in `example-backend/`. **M8 (developer on-ramp) is built**: `conduit://add-agent` deep links (`DeepLinkParser`, scheme in `Info.plist`, routed via `AppEnvironment.pendingAgentLink` → Contacts tab; re-scan of a known pairing endpoint opens Edit) with `scripts/pair.py` printing the link + terminal QR (key embedded by default — the link is then a secret); a **staged Test Connection** checklist (pairing → credentials → transport → ready, injectable `pairingResolver` seam); **persisted failure reasons** (`CallLogEntry.failureReasonRaw` → "Failed — authentication" rows + an in-call hint); and **decluttered one-line footers** linking to the published `docs/using-conduit.md`. Device-verified June 2026 (Camera-scan QR → prefilled sheet → staged diagnostics against the deployed engine; migration sane over existing history). **M9 (SmallWebRTC) is built and device-verified**: `TransportKind.smallwebrtc` + `PipecatSmallWebRTCTransport` (Daily-adapter mirror; pairing returns the engine's offer URL + a short-lived bearer as `room_url`/`token`, `PairingClient` unchanged), the engine's `POST/PATCH /webrtc/{agent_id}/offer` running **in-process bots** (offer = rendezvous; dropped peer → immediate `end_now`; `run_bot` gained `handle_sigterm`), and the zero-cloud LAN path (`pair.py --transport smallwebrtc`, `NSLocalNetworkUsageDescription` for the iOS local-network prompt). **Critical:** the stock package's stasel/WebRTC collides with Daily's embedded WebRTC (un-prefixed `RTC*` duplicated → every Daily call crashed; see `bugs.md`), so `project.yml` pins **our fork** `AmirAlsad/pipecat-client-ios-small-webrtc@livekit-webrtc` built against `LiveKitWebRTC` (typealias shim; upstream issue pipecat-client-ios-small-webrtc#6) — after any transport-SDK bump, confirm no stasel `WebRTC.framework` in the device bundle and regression-call Daily. SmallWebRTC is LAN/self-hosted-only (UDP media — not Railway); verified over hotspot LAN: pairing → two-way audio → clean RTVI end-call → instant teardown, with Daily + LiveKit regression calls green in the same binary. **M10 Siri dialing (layers 1–2) is built**: `Core/Services/Siri/` — `CallAgentIntent`/`AgentEntity`/`ConduitAppShortcuts` (App Intents phrases, `openAppWhenRun = true` because CallKit rejects background `CXStartCallAction`) and `SiriCallHandler` (SiriKit `INStartCallIntent`, in-app, `.continueInApp`), both resolving via the pure `AgentNameMatcher` and funneling through `AppEnvironment.pendingSiriCall`, consumed by `RootTabView` **only at `scenePhase == .active`** (cold-launch race). Vocabulary refresh hooks at launch + save + both deletes; `INPreferences.requestSiriAuthorization` at launch (else the Settings Siri toggle defaults off). Device-verified unlocked on both layers; SiriKit dials from the lock screen, the App Shortcut path can't (must open the app). `CallAgentIntent`/`agent` parameter names are frozen for the iOS 27 `.phone.startCall` schema adoption (layer 3 — needs the Xcode 27 SDK; in-car steering-wheel verification also outstanding). See [`docs/CORE_SYSTEMS.md`](./docs/CORE_SYSTEMS.md). The repo is **public**, with a user-facing **docs site** (MkDocs Material → GitHub Pages via `.github/workflows/docs.yml`; published pages listed in `docs/README.md`) that the app links to through `Core/Utilities/ExternalLinks.swift` — keep `mkdocs.yml` nav slugs and `ExternalLinks` in sync. `PairingClient` accepts `url` as a room-URL fallback key, and the reference backend's per-agent routes (`/connect/{agent_id}`) match the contract's "endpoint identifies the agent" model.
