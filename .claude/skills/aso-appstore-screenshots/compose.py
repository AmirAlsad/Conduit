...
# ── Fonts ───────────────────────────────────────────────────────────
# Bundled, redistributable OFL fonts live in assets/fonts/. The skill
# auto-picks one to match the app's personality and passes --font <key>.
FONTS_DIR = os.path.join(os.path.dirname(__file__), "assets", "fonts")
FONT_REGISTRY = {
    "inter":      "Inter-Black.ttf",          # neutral grotesque — finance, productivity
    "montserrat": "Montserrat-Black.ttf",     # geometric — tech, modern
    "poppins":    "Poppins-Black.ttf",        # friendly rounded — social, lifestyle, kids
    "oswald":     "Oswald-Bold.ttf",          # condensed — sports, news, fitness
    "bebas":      "BebasNeue-Regular.ttf",    # bold display — fitness, energy, action
    "archivo":    "ArchivoBlack-Regular.ttf", # authoritative grotesque — bold/utility
    "playfair":   "PlayfairDisplay-Black.ttf",# elegant serif — luxury, reading, fashion
}
DEFAULT_FONT_KEY = "inter"
# macOS-only Apple font — used only as a convenience fallback when present.
SF_PRO_BLACK = "/Library/Fonts/SF-Pro-Display-Black.otf"

# Resolved font path (overridden in main via resolve_font). None => PIL default.
FONT_PATH = os.path.join(FONTS_DIR, FONT_REGISTRY[DEFAULT_FONT_KEY])


def resolve_font(spec):
    """Resolve a --font value (bundled key, file path, or None) to a usable
    font path. Fallback chain: requested -> SF Pro (if present) -> bundled
    Inter Black -> PIL default (None)."""
    if spec:
        ...
        if os.path.isfile(spec):
            return spec
        print(f"⚠ font '{spec}' not found; falling back", flush=True)
    if os.path.isfile(SF_PRO_BLACK):
        return SF_PRO_BLACK
    bundled = os.path.join(FONTS_DIR, FONT_REGISTRY[DEFAULT_FONT_KEY])
...
def load_font(size):
    """Load the resolved font at a size, or PIL's default if unavailable."""
    if FONT_PATH:
...
def compose(bg_hex, verb, desc, screenshot_path, output_path):
    ...
    font_name = os.path.basename(FONT_PATH) if FONT_PATH else "PIL-default"
    print(f"✓ {output_path} ({CANVAS_W}×{CANVAS_H}, font: {font_name})")
...
def main():
    ...
    p.add_argument(
        "--font",
        default=None,
        help="Headline font: a bundled key (%s) or a path to a .ttf/.otf. "
        "Defaults to bundled Inter Black." % "|".join(FONT_REGISTRY),
    )
    p.add_argument(
        "--list-fonts",
        action="store_true",
        help="List bundled font keys and exit",
    )
    args = p.parse_args()

    if args.list_fonts:
        for key, fname in FONT_REGISTRY.items():
            print(f"{key:12s} {fname}")
        return

    missing = [
        f"--{n}"
        for n in ("bg", "verb", "desc", "screenshot", "output")
